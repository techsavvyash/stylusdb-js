...
server.on("data", async (pkt) => {
  if (pkt === undefined) {
    proxySocket.write("+undefined packet received");
  }
  console.log("***************************************");
  console.log("***************************************");
        ...
        case "SET":
          ...
          try {
            if (data && data.length > 0) {
              ...
              proxySocket.write(`+${JSON.stringify(cmd)} - ack`);
            } else {
              proxySocket.write("+Error 2: Invalid data format");
            }
          } catch (e) {
            console.log(e);
            proxySocket.write(`+${e}`);
        ...
        case "GET":
          try {
            if (data && data.length > 0) {
              ...
              proxySocket.write(`+Value of key : ${cmd.key} is ${val}`);
            } else {
              proxySocket.write("+Invalid data format");
            }
          } catch (e) {
            console.log(e);
            proxySocket.write(`+${e}`);
      ...
      switch (task) {
        ...
        default:
          if (task === undefined) {
            ...
            proxySocket.write("+one of the packets was with undefined task");
          }
...
server.listen(raftNodeServerPort, () => {
  ...
  proxySocket = net.createConnection({ port: 6767 }, () => {
